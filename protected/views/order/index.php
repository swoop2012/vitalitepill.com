    ...
    <div class="delivery-payment b-delivery">
        ...
        <? if(!empty($delivery)):?>
            ...
            <? foreach($delivery as $key=>$value):?>
                <li data-id="<?= $value->id;?>" data-type="<?= $value->Type;?>" <?=$key==$count-1?'class="fl"':''?> data-price='<?= Calculate::DiscountDelivery($order['totalSum'],$value->Price,$value->FreeIf);?>' data-name='<?= $value->Name;?>'>
                    ...
                    <div class="payment-select">
                        ...
                        <ul>
                            ...
                            <? foreach($value->payment_api as $key=>$deliverypayment):?>
                                <li data-id="<?= $deliverypayment->id;?>" <?=$key==$count-1?'class="fl"':''?> data-name='<?= $deliverypayment->Name;?>'>
