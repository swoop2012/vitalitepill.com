<?php
    ...
    {  
        ...
        {  
            ...
            {  
                ...
                curl_setopt($conn[$i], CURLOPT_SSL_VERIFYPEER, false);//В большенстве случаев помогает, если используется https
                curl_setopt($conn[$i], CURLOPT_SSL_VERIFYHOST, false);//В большенстве случаев помогает, если используется https
    ...
    function multigetArray($urls, &$result)  
    {  
        $useragent = $this->all_useragents[array_rand($this->all_useragents)];  
	...
	{  
	    $conn[$key]=curl_init(trim($url));  
	    curl_setopt($conn[$key],CURLOPT_RETURNTRANSFER, 1);  
	    curl_setopt($conn[$key],CURLOPT_TIMEOUT, $this->timeout);  
	    curl_setopt($conn[$key],CURLOPT_USERAGENT, $useragent);
        curl_setopt($conn[$key], CURLOPT_SSL_VERIFYPEER, false);//В большенстве случаев помогает, если используется https
        curl_setopt($conn[$key], CURLOPT_SSL_VERIFYHOST, false);//В большенстве случаев помогает, если используется https
	    curl_multi_add_handle ($mh,$conn[$key]);  
	}  
	do { $n=curl_multi_exec($mh,$active); usleep(100); } while ($active);  
	foreach ($urls as $key => $url)  
	{  
	    $result[$key]=curl_multi_getcontent($conn[$key]);  
	    curl_close($conn[$key]);  
	}  
	curl_multi_close($mh);  
        ...
        function multigetShares($urls, &$result)
        {
            $useragent = $this->all_useragents[array_rand($this->all_useragents)];
        ...
        foreach ($urls as $key => $value)
        {
            ...
            {
            $conn[$key][$network]=curl_init(trim($url));
            curl_setopt($conn[$key][$network],CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($conn[$key][$network],CURLOPT_TIMEOUT, $this->timeout);
            curl_setopt($conn[$key][$network],CURLOPT_USERAGENT, $useragent);
            curl_setopt($conn[$key][$network], CURLOPT_SSL_VERIFYPEER, false);//В большенстве случаев помогает, если используется https
            curl_setopt($conn[$key][$network], CURLOPT_SSL_VERIFYHOST, false);//В большенстве случаев помогает, если используется https

            curl_multi_add_handle ($mh,$conn[$key][$network]);
            }
        }
        do { $n=curl_multi_exec($mh,$active); usleep(100); } while ($active);
        foreach ($urls as $key => $value)
        {   foreach($value as $network=>$url)
            {
            $result[$key][$network]=curl_multi_getcontent($conn[$key][$network]);
            curl_close($conn[$key][$network]);
            }
